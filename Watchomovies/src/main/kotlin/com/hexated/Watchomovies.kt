...
class Watchomovies : MainAPI() {
    override var mainUrl = "https://watchomovies.baby"
    ...
    override val mainPage = mainPageOf(
        ...
        "$mainUrl/genre/tv-shows/page/" to "Desi",
        "$mainUrl/genre/hollywood/page/" to "Erotic Movies",
        "$mainUrl/genre/xxx-scenes/page/" to "Brazzers Videos",
        "$mainUrl/genre/parody/page/" to "Parody Movies",
        ...
        if (data.contains("watchomovies")) {
            ...
            doc.select("div.movieplay iframe").map { fixUrl(it.attr("src")) }
                .apmap { source ->
                    safeApiCall {
                        when {
                            source.startsWith("https://embed.net") -> app.get(
                                source,
                                referer = "$directUrl/"
